...
Set `"then": "screenshot"` on `input_window` for an immediate post-batch PNG at the default capture size, or customize delay/crop/size using nested `observation` options. The screenshot runs only after a completed batch and uses a fresh observation check and fresh geometry. Under the existing grant model, control and record grants include observation; observation alone never includes control or recording. There is no new permission bypass.

The result metadata is available in both MCP `structuredContent` and a JSON text block, including runtime failures and approval responses. Images are additional content blocks. The result retains `status: completed`, `actions` and `window_id`, with nested `observation` metadata and PNG content. If capture fails or permission has gone away, `observation.status` is `failed` (or `approval_required` with a request ID), but the **input remains completed**. Retry observation, not the batch. The default still captures immediately. Input delivery can finish before the application renders: a stale screenshot is not evidence that the input failed. Set `observation.delay_ms` to **0–5000 milliseconds** (default **0**) to give the app time before capture, for example `"observation": {"delay_ms": 200}`. This option is valid only with `then: "screenshot"`; invalid values are rejected before input. It is a cancellable broker-side wait after the input lock and synthetic state are released, followed by the normal fresh permission/session/geometry checks. Cancellation during the wait retains completed input and reports a failed observation. The delay is additional to the input execution budget, bounded to five seconds, and is **not a settled-frame guarantee**. See [post-action observation](OBSERVATION_DELAY.md) for the contract and real-application test recipe.

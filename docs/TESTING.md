...
The public `v0.1.0` release completed the hosted test and release workflows. GoReleaser published amd64/arm64 archives plus `checksums.txt`; both downloaded archives matched the manifest and contained the executable, documentation, notices, and protocol XML. Clean Go 1.25 containers successfully ran both `go install github.com/samsaffron/hyprland-computer-use/cmd/hyprland-computer-use@v0.1.0` and `@latest`, with module metadata resolving to canonical version `v0.1.0`. In a clean pinned Arch container, both the latest and `--version v0.1.0` installer paths reported the expected embedded version and their installed release executables completed `setup --build-only --input-mode=auto`, compiling both native input variants against Hyprland 0.56.2. Live plugin replacement and `serve` startup remain part of the disposable-session gate, not container evidence.

...
- Optional post-input observation delay (0–5000ms) runs in the broker after input transactions and the input lock are released. It grants no authority or held input; capture rechecks current permission, session and geometry after the wait. Cancellation affects observation, not the already completed input result. A delay is not a rendering fence.
